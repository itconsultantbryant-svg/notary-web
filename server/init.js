...
async function migrateCmsSlug(db) {
  const old = await db.get("SELECT slug FROM cms_pages WHERE slug = ?", ["index"]);
  const home = await db.get("SELECT slug FROM cms_pages WHERE slug = ?", ["home"]);
  if (old && !home) {
    await db.run("UPDATE cms_pages SET slug = ? WHERE slug = ?", ["home", "index"]);
    await db.run("UPDATE cms_blocks SET page_slug = ? WHERE page_slug = ?", ["home", "index"]);
  }
}

async function seedCms(db) {
  await migrateCmsSlug(db);
...
async function initializeDatabase() {
  ...
  await seedCms(db);
  const docs = await seedDocuments(db);
  return { admin, documentsCreated: docs };
}

module.exports = { initializeDatabase, CMS_SEED };
